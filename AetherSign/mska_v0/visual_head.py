from __future__ import annotations

import torch
import torch.nn as nn
import torch.nn.functional as F

from .utils import MLPHead, MaskedNorm, PositionalEncoding, PositionwiseFeedForward


class VisualHead(torch.nn.Module):
    def __init__(
        self,
        cls_num,
        input_size=512,
        hidden_size=1024,
        ff_size=2048,
        pe=True,
        ff_kernelsize=None,
        pretrained_ckpt=None,
        is_empty=False,
        frozen=False,
        plus_conv_cfg=None,
        ssl_projection_cfg=None,
    ):
        super().__init__()
        ff_kernelsize = ff_kernelsize or [3, 3]
        plus_conv_cfg = plus_conv_cfg or {}
        ssl_projection_cfg = ssl_projection_cfg or {}

        self.is_empty = is_empty
        self.plus_conv_cfg = plus_conv_cfg
        self.ssl_projection_cfg = ssl_projection_cfg

        if not is_empty:
            self.frozen = frozen
            self.hidden_size = hidden_size

            self.fc1 = nn.Identity() if input_size is None else torch.nn.Linear(input_size, hidden_size)
            self.bn1 = MaskedNorm(num_features=hidden_size, norm_type="batch")
            self.relu1 = torch.nn.ReLU()
            self.dropout1 = torch.nn.Dropout(p=0.1)
            self.pe = PositionalEncoding(hidden_size) if pe else torch.nn.Identity()
            self.feedforward = PositionwiseFeedForward(
                input_size=hidden_size,
                ff_size=ff_size,
                dropout=0.1,
                kernel_size=ff_kernelsize,
                skip_connection=True,
            )
            self.layer_norm = torch.nn.LayerNorm(hidden_size, eps=1e-6)

            if plus_conv_cfg:
                convs = []
                for _ in range(plus_conv_cfg["num_layer"]):
                    convs.append(
                        nn.Conv1d(
                            hidden_size,
                            hidden_size,
                            kernel_size=plus_conv_cfg["kernel_size"],
                            stride=plus_conv_cfg["stride"],
                            padding="same",
                        )
                    )
                self.plus_conv = nn.Sequential(*convs)
            else:
                self.plus_conv = nn.Identity()

            if ssl_projection_cfg:
                self.ssl_projection = MLPHead(
                    embedding_size=hidden_size,
                    projection_hidden_size=ssl_projection_cfg["hidden_size"],
                )
            else:
                self.ssl_projection = None

            self.gloss_output_layer = torch.nn.Linear(hidden_size, cls_num)

            if frozen:
                self.frozen_layers = [
                    self.fc1,
                    self.bn1,
                    self.relu1,
                    self.pe,
                    self.dropout1,
                    self.feedforward,
                    self.layer_norm,
                ]
                for layer in self.frozen_layers:
                    for _, param in layer.named_parameters():
                        param.requires_grad = False
                    layer.eval()
        else:
            self.gloss_output_layer = torch.nn.Linear(input_size, cls_num)

        if pretrained_ckpt:
            raise NotImplementedError("pretrained_ckpt loading is not implemented in the self-contained v0 package")

    def forward(self, x, mask, valid_len_in=None):
        batch_size, time_steps, _ = x.shape
        if not self.is_empty:
            if not self.frozen:
                x = self.fc1(x)
                x = self.bn1(x, mask)
                x = self.relu1(x)
                x = self.pe(x)
                x = self.dropout1(x)
                x = self.feedforward(x)
                x = self.layer_norm(x)
                x = self.plus_conv(x.transpose(1, 2)).transpose(1, 2)
            else:
                with torch.no_grad():
                    for index, layer in enumerate(self.frozen_layers):
                        if index == 1:
                            x = layer(x, mask)
                        else:
                            x = layer(x)
                x = self.plus_conv(x.transpose(1, 2)).transpose(1, 2)

        logits = self.gloss_output_layer(x)
        gloss_probabilities_log = logits.log_softmax(2)
        gloss_probabilities = logits.softmax(2)

        if self.plus_conv_cfg:
            _, output_steps, _ = x.shape
            valid_len_out = torch.floor(valid_len_in * output_steps / time_steps).long()
        else:
            valid_len_out = valid_len_in

        if self.ssl_projection_cfg:
            x_ssl = self.ssl_projection(x)
            if self.ssl_projection_cfg.get("normalize", False):
                x_ssl = F.normalize(x_ssl, dim=-1)
        else:
            x_ssl = None

        return {
            "gloss_feature_ssl": x_ssl,
            "gloss_feature": x,
            "gloss_feature_norm": F.normalize(x, dim=-1),
            "gloss_logits": logits,
            "gloss_probabilities_log": gloss_probabilities_log,
            "gloss_probabilities": gloss_probabilities,
            "valid_len_out": valid_len_out,
        }
